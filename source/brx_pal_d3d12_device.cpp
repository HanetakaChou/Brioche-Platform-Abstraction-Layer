...
{
    void *new_unwrapped_render_pass_base = mcrt_malloc(sizeof(brx_pal_d3d12_render_pass), alignof(brx_pal_d3d12_render_pass));
    assert(NULL != new_unwrapped_render_pass_base);

    brx_pal_d3d12_render_pass *new_unwrapped_render_pass = new (new_unwrapped_render_pass_base) brx_pal_d3d12_render_pass{};
    new_unwrapped_render_pass->init(color_attachment_count, color_attachments, depth_stencil_attachment);

    return new_unwrapped_render_pass;
}

void brx_pal_d3d12_device::destroy_render_pass(brx_pal_render_pass *wrapped_render_pass) const
{
    assert(NULL != wrapped_render_pass);
    brx_pal_d3d12_render_pass *delete_unwrapped_render_pass = static_cast<brx_pal_d3d12_render_pass *>(wrapped_render_pass);

    delete_unwrapped_render_pass->uninit();

    delete_unwrapped_render_pass->~brx_pal_d3d12_render_pass();
    mcrt_free(delete_unwrapped_render_pass);
}
...
brx_pal_frame_buffer *brx_pal_d3d12_device::create_frame_buffer(brx_pal_render_pass const *wrapped_render_pass, uint32_t width, uint32_t height, uint32_t color_attachment_count, brx_pal_color_attachment_image const *const *color_attachments, brx_pal_depth_stencil_attachment_image const *depth_stencil_attachment) const
{
    void *new_unwrapped_frame_buffer_base = mcrt_malloc(sizeof(brx_pal_d3d12_frame_buffer), alignof(brx_pal_d3d12_frame_buffer));
    assert(NULL != new_unwrapped_frame_buffer_base);

    brx_pal_d3d12_frame_buffer *new_unwrapped_frame_buffer = new (new_unwrapped_frame_buffer_base) brx_pal_d3d12_frame_buffer{};
    new_unwrapped_frame_buffer->init(wrapped_render_pass, width, height, color_attachment_count, color_attachments, depth_stencil_attachment);
    return new_unwrapped_frame_buffer;
}

void brx_pal_d3d12_device::destroy_frame_buffer(brx_pal_frame_buffer *wrapped_frame_buffer) const
{
    assert(NULL != wrapped_frame_buffer);
    brx_pal_d3d12_frame_buffer *delete_wrapped_frame_buffer = static_cast<brx_pal_d3d12_frame_buffer *>(wrapped_frame_buffer);

    delete_wrapped_frame_buffer->uninit();

    delete_wrapped_frame_buffer->~brx_pal_d3d12_frame_buffer();
    mcrt_free(delete_wrapped_frame_buffer);
}
...
brx_pal_storage_image *brx_pal_d3d12_device::create_storage_image(BRX_PAL_STORAGE_IMAGE_FORMAT wrapped_storage_image_format, uint32_t width, uint32_t height, bool volume, uint32_t depth, bool allow_sampled_image) const
{
    ...
    new_unwrapped_storage_image->init(this->m_memory_allocator, this->m_storage_intermediate_image_memory_pool, wrapped_storage_image_format, width, height, volume, depth, allow_sampled_image);
